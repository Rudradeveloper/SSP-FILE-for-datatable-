...
class CustomeSSP {
    ...
    static function order($request, $columns, $isJoin = false) {
        ...
        if (isset($request['order']) && count($request['order'])) {
            $orderBy = array();
            $dtColumns = CustomeSSP::pluck($columns, 'dt');
        ...
        if (isset($request['search']) && $request['search']['value'] != '') {
            ...
            for ($i = 0, $ien = count($request['columns']); $i < $ien; $i++) {
                ...
                if ($requestColumn['searchable'] == 'true') {
                    $binding = CustomeSSP::bind($bindings, '%' . $str . '%', PDO::PARAM_STR);
     ...
     * in response to an CustomeSSP request, or can be modified if needed before
    ...
    static function simple($request, $sql_details, $table, $primaryKey, $columns, $joinQuery = NULL, $extraWhere = '', $groupBy = '', $having = '') {
        ...
        $bindings = array();
        $db = CustomeSSP::sql_connect($sql_details);
        // Build the SQL query string from the request
        $limit = CustomeSSP::limit($request, $columns);
        $order = CustomeSSP::order($request, $columns, $joinQuery);
        $where = CustomeSSP::filter($request, $columns, $bindings, $joinQuery);
        ...
        if ($joinQuery) {
            $col = CustomeSSP::pluck($columns, 'db', $joinQuery);
            ...
            $query = "SELECT SQL_CALC_FOUND_ROWS `" . implode("`, `", CustomeSSP::pluck($columns, 'db')) . "`
			 FROM `$table`
			 $where
			 $extraWhere
			 $groupBy
        ...
        $data = CustomeSSP::sql_exec($db, $bindings, $query);
        // Data set length after filtering
        $resFilterLength = CustomeSSP::sql_exec($db, "SELECT FOUND_ROWS()"
        );
        $recordsFiltered = $resFilterLength[0][0];
        // Total data set length
        $resTotalLength = CustomeSSP::sql_exec($db, "SELECT COUNT(`{$primaryKey}`)
			 FROM   `$table`"
        );
        ...
        return array(
            ...
            "data" => CustomeSSP::data_output($columns, $data, $joinQuery)
        );
    }
    ...
    static function sql_exec($db, $bindings, $sql = null) {
        ...
        try {
            ...
            CustomeSSP::fatal("An SQL error occurred: " . $e->getMessage());
        }
        // Return all
